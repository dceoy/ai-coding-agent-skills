...
- Every accepted subagent dispatch must be bounded by an explicit finite deadline supplied by the caller or enforced by the runtime; the orchestrator must not invent, shorten, or override it. If neither provides a usable deadline, report the phase as `unsupported` before dispatch. A still-running poll is not failure; keep waiting on the same accepted dispatch until it reaches a terminal result or the deadline expires. On terminal failure or expiry, stop or cancel and reap it without launching replacement work.
