...
2. Set the scan scope from the invocation: when repository-wide simplification is explicitly requested, scan the whole repository, excluding generated, vendored, cache, and build-output trees; when invoked proactively or for a narrower task, start with code touched in the current task/session or otherwise in scope, and expand only as needed to prove simplifications safe.
3. Within that scope, look for dead material, duplicated logic/declarations, needless indirection, speculative abstractions, obsolete compatibility paths, and redundant tests or documentation. Prove removals by searching source, tests, docs, scripts, CI, packaging, configuration, and entry points as needed; account for dynamic loading, reflection, plugin discovery, command names, serialized formats, environment variables, and external APIs. Keep uncertain contracts unless breaking them is explicitly allowed.
