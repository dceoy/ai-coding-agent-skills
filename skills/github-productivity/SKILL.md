...
- Exit codes: `0` success, `1` the run was `incomplete` or `paused` (fail closed; committed state is unchanged), `2` invalid arguments — including a negative `--overlap-hours` or reusing a `--workdir` for a different `--org` than it already has evidence for — `3` the workdir is already locked by another collection run, including by a since-killed process (see [Known limitations](references/methodology.md#known-limitations) for recovery). A retryable failure (rate limit, transient transport error) leaves a durable pending-generation checkpoint (`<workdir>/.collect.pending.json`); rerunning `collect` with the same arguments resumes that generation from its completed shards instead of restarting discovery. See [Resumable generations and shard checkpoints](references/methodology.md#resumable-generations-and-shard-checkpoints).
