...
- No automatic stale-lock recovery in v1. Each `gh api` call is bounded (120s), so a wedged call fails that run rather than hanging forever, but if the `collect` process itself is killed (SIGKILL, host loss) while holding the lock, `<workdir>/.collect.lock` is left in place and every subsequent `collect` fails closed (exit code `3`) until an operator manually removes that file. Manual removal is safe precisely because no run can be committed without holding the lock through the full transaction: whatever the killed run had done, the next run repeats the required overlap/backfill from the last actually committed state.
