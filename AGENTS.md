...
6. Start exactly one implementation agent:
   ...
   - Use `worker_terra` only after all material decisions are settled and the contract defines a bounded ownership zone plus an explicit Terra condition: broad diagnosis where the exact affected files inside that zone are not yet known; cross-cutting adaptation whose exact file set must be discovered inside that zone; implementation or validation requiring adaptive engineering judgment despite concrete acceptance criteria; or a documented Luna escalation exposing one of those conditions.
...
11. After parent verification, capture a second repository baseline immediately before final review. Spawn a fresh `advisor_sol` context and provide the user goal, allowed change set, the complete pre-worker-baseline-relative diff or explicit immutable base and head revisions, interfaces and constraints, and the main session's verification evidence.
