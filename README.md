...
This named-agent setup targets Codex CLI 0.148.0 or later. Codex 0.148.0 includes upstream OSS support for combining an explicit named role with `fork_turns: "all"`, but the active app/native runtime may expose a different capability surface. The planner routing therefore prefers full-history inheritance and falls back to the same named planner with explicit context only when the runtime explicitly rejects that combination.
...
- **Codex CLI** - For `.agents/skills/`; Codex CLI 0.148.0+ is the target release line for `.codex/agents/`, while planner full-history compatibility is still probed at runtime
