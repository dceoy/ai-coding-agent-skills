...
For non-trivial changes, invoke `planner` and `advisor` with `fork_turns: "none"`, pass each role only the task-specific context it needs, and require read-only behavior. The checked-in TOML definitions request read-only sandboxes and remain authoritative for the configured named role, model, reasoning effort, and sandbox default, but a broader effective runtime sandbox such as `workspace-write` is not by itself a failure. Resolve material decisions before main-agent implementation, run verification, and report completion only after an independent `VERDICT: ship`.

Invoke these roles only through native multi-agent dispatch. Report `unsupported` only when native dispatch is unavailable or runtime evidence explicitly shows a fallback, incompatible model or reasoning override, or inherited context contrary to the requested isolation; missing runtime telemetry or a writable effective sandbox alone is not a failure. The named agents must not modify files regardless of available write capability. Reject a result when the post-dispatch Git-visible state differs from its recorded baseline or available runtime evidence shows a mutating action, including a transient edit later restored. This guard protects persistent Git-visible state but does not attest that a writable runtime performed no transient writes. Do not fall back to nested `codex exec`, shell wrappers, copied prompts, or generic agents.
