...
Models and reasoning effort are intentionally unpinned in the TOML files and selected at dispatch time. Codex subagents use GPT-6 models only: planner=Sol, advisor=Sol, lightweight docs/tests review=Luna, substantive correctness/security/performance review=Sol, and feedback-analyst=Luna, with Astra reserved for the hardest escalation cases defined in `.codex/AGENTS.md`. Effort remains task-sensitive: Luna=`max`; Sol uses `xhigh` for routine planning, ordinary correctness/code-reasoning review, and ambiguous/code-heavy feedback triage, `high` for advisor and architecture/security/high-risk bands, and `max` only for the hardest quality-first work; Astra=`medium|high|xhigh|max`. Implementation remains in the top-level main agent.
