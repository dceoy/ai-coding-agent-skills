...
def test_initial_backfill_stops_at_boundary_via_descending_sort(
    ...
    fake_gh.set_list(
        ...
        [[make_pr(5, "2026-01-10T00:00:00Z"), make_pr(4, "2026-01-06T00:00:00Z")]],
        sort="updated",
        direction="desc",
    )
    fake_gh.set_object(
...
def test_incremental_discovery_uses_created_ascending_since_watermark(
    ...
    fake_gh.set_list(
        ...
        [[make_pr(9, "2026-01-06T00:00:00Z")]],
        sort="created",
        direction="asc",
    )
    fake_gh.set_object(
...
def test_reconciliation_adds_pr_missed_by_incremental_scan(
    ...
    fake_gh.set_list(
        "/repos/acme/repo1/issues",
        [[make_pr(10, "2026-01-06T00:00:00Z")]],
        sort="created",
        direction="asc",
    )
    fake_gh.set_list(
        "/repos/acme/repo1/issues",
        [[make_pr(7, "2026-01-06T00:00:00Z")]],
        sort="updated",
        direction="asc",
    )
    fake_gh.set_object(
...
def test_non_pr_issues_are_filtered_out(tmp_path: Path, fake_gh: FakeGh) -> None:
    ...
    fake_gh.set_list(
        ...
        [
            [
                make_pr(11, "2026-01-08T00:00:00Z", is_pr=False),
                make_pr(12, "2026-01-08T00:00:00Z"),
            ]
        ],
        sort="updated",
        direction="asc",
...
def test_failed_bundle_fetch_leaves_committed_state_unchanged(
    ...
    fake_gh.set_list(
        "/repos/acme/repo1/issues",
        [[make_pr(20, "2026-01-15T00:00:00Z")]],
        sort="created",
        direction="asc",
    )
...
def test_archived_and_fork_repositories_are_retained(
    ...
    fake_gh.set_list(
        ...
        [
            [
                make_repo(1, "archived-repo", archived=True),
                make_repo(2, "fork-repo", fork=True),
            ]
        ],
    )
    fake_gh.set_list(
...
def test_repository_rename_preserves_single_state_entry(
    ...
    fake_gh.set_list("/orgs/acme/repos", [[make_repo(1, "old-name")]])
    fake_gh.set_list(
        "/repos/acme/old-name/pulls", [[]], sort="updated", direction="desc"
    )
    collect.run_collect(org="acme", workdir_path=tmp_path, start=_START, end=_END)

    fake_gh.set_list("/orgs/acme/repos", [[make_repo(1, "new-name")]])
    fake_gh.set_list(
...
def test_watermark_advances_to_refresh_started_at_not_max_updated_at(
    ...
    fake_gh.set_list(
        "/repos/acme/repo1/pulls",
        [[make_pr(1, "2030-06-15T00:00:00Z")]],
        sort="updated",
        direction="desc",
    )
    fake_gh.set_object(
...
def test_manifest_records_collector_revision(tmp_path: Path, fake_gh: FakeGh) -> None:
...
@pytest.mark.parametrize(
    "resume_org", ["acme", "ACME"], ids=["same-case", "different-case"]
)
def test_rate_limit_resumes_at_first_unfinished_repository(
    tmp_path: Path, fake_gh: FakeGh, monkeypatch: pytest.MonkeyPatch, resume_org: str
) -> None:
    """A later repository rate limit does not replay completed repositories.

    Resuming with a differently-cased organization login must still bind to
    the same paused checkpoint rather than discarding it.
    """
    fake_gh.set_list(
        "/orgs/acme/repos",
    ...
    second = collect.run_collect(
        org=resume_org, workdir_path=tmp_path, start=_START, end=_END
    )
    assert second.status == "complete"
...
def test_rate_limit_resumes_at_first_unfinished_pr_bundle(
    ...
    def flaky_request(**kwargs: Any) -> Any:  # noqa: ANN401
        nonlocal failed
        endpoint = str(kwargs["endpoint"])
...
def test_resume_rebuilds_stale_seal_after_new_shards_complete(
    ...
    collect._materialize_generation_raw(tmp_path, checkpoint)
...
def test_completed_shards_are_synced_before_checkpoint(
    ...
    real_write = collect._write_checkpoint

    def recording_sync(workdir_path: Path, run_id: str) -> None:
        real_sync(workdir_path, run_id)
        synced.add(run_id)

    def checked_write(workdir_path: Path, checkpoint: dict[str, Any]) -> None:
    ...
    monkeypatch.setattr(collect, "_write_checkpoint", checked_write)
...
def test_non_retryable_failure_finalizes_incomplete_generation(
    ...
    def failing_paginate(**kwargs: Any) -> Any:  # noqa: ANN401
...
def _assert_completed_shards_synced(
    checkpoint: dict[str, Any], synced: set[str]
) -> None:
