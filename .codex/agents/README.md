...
The planner intentionally inherits the full parent conversation with `fork_turns: "all"`. This more closely matches Claude Code's OpusPlan continuity: the higher-capability planning phase sees the user's request, prior decisions, and accumulated context directly instead of relying on a reconstructed handoff packet. The advisor intentionally remains different: it uses `fork_turns: "none"` so a second opinion starts from a fresh context and reviews the explicit planner contract and primary evidence independently.
...
`planner` inherits the full parent conversation when invoked with `fork_turns: "all"`. Treat that inherited history as the authoritative source for the user's request, prior decisions, constraints, and unresolved questions rather than reconstructing them into a mandatory context packet.

The dispatch should state the planning task and add only task-specific information that is not already present in the inherited conversation, such as newly inspected repository state, relevant implementation evidence, or a newer fact that supersedes stale context. Do not replace inherited history with an implementation-oriented summary.
...
1. Apply the named-agent mutation guard, select and pass the planner reasoning effort according to the adaptive policy above, then invoke `planner` with `fork_turns: "all"` in a behaviorally read-only child context. Let inherited conversation history carry the user's request, prior decisions, constraints, and open questions; add only newly inspected repository state or task-specific evidence needed for planning. Prefer its configured read-only sandbox, but accept its contract unless runtime evidence explicitly reports a fallback, incompatible model override, failure to honor the explicitly requested reasoning effort, failure to inherit the requested parent turns, or a writable invocation outside a Git worktree; missing runtime telemetry, an adaptively selected reasoning effort, or a writable effective sandbox alone is not a failure when the guard can be established.
