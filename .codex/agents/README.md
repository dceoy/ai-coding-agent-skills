...
External-input records identify each input without exposing plaintext secrets, explain how it influences verification, and contain a reproducible digest, immutable identifier, or other independently comparable attestation. Every external input must be frozen, replayable, or independently attestable. An input that cannot be safely identified and compared blocks verification and final review.
