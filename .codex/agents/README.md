...
Both named roles use `fork_turns: "none"` so the routing has one deterministic context contract across runtimes. Planner correctness comes from an explicit context packet rather than inherited parent history. Advisor independence comes from a fresh context that reviews task-specific primary evidence instead of inheriting the parent agent's conclusions.
